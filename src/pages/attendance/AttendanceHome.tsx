  ...
  fetch(`${BASE}/interngo/users`)
    ...
    .then((users) => {
      const yearMap: Record<
        string,
        Map<string, { norm: string; display: string }>
      > = {};
      ...
      users
        ...
        .forEach((u: any) => {
          ...
          if (!yearMap[year].has(norm)) {
            // 🔥 Create ONE standard display label
            const num = rawBatch.replace(/[^0-9]/g, "");
            const display = num ? `Batch-${num}` : rawBatch;

            yearMap[year].set(norm, {
              norm,
              display,
            });
          }
      ...
      setGroups(
        Object.keys(yearMap)
          ...
          .map((year) => ({
            year,
            batches: Array.from(yearMap[year].values()).map(
              (b) => b.display
            ),
          }))
      );
...
//   // ---------------- LOAD YEAR + BATCH ----------------
//   useEffect(() => {
//     fetch(`${BASE}/interngo/getallprofiles`)
//       .then((r) => r.json())
