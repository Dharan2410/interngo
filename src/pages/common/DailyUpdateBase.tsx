...
useEffect(() => {
  ...
  const loadDaily = async () => {
    try {
      ...
      // 1️⃣ Fetch flat task rows (TaskRow[])
      const records = await fetchBatchDaily(selectedYear, iso);

      // 2️⃣ Group tasks by userId
      const groupedByUser: Record<string, TaskRow[]> = {};
      records.forEach((task: any) => {
        const uid = String(task.userId);
        if (!groupedByUser[uid]) groupedByUser[uid] = [];
        groupedByUser[uid].push(task);
      });

      // 3️⃣ Filter interns by year + batch (Mongo profiles)
      const profMatches = professionalInfoList.filter(
        (p) =>
          String(p.year) === String(selectedYear) &&
      ...
      // 4️⃣ Build intern list + task map
      profMatches.forEach((p) => {
        const uid = String(p._id);

        mapTasks[uid] = groupedByUser[uid] || [];

        interns.push({
          id: uid,
          name: p.name || "Unknown Intern",     // ✅ FIXED
          totalTime: "0h 00m",                  // optional calc later
          role: p.designation || p.role || "intern",
        });
      });
  ...
  loadDaily();
}, [selectedYear, selectedBatch, selectedDate, professionalInfoList]);
