...
// const keyFor = (userId: string, dateIso: string) =>
//   `daily_updates_${userId}_${dateIso}`;
...
//   // Fetch & save using localStorage
//   const fetchTasksFromBackend = async (userId: string, dateIso: string) => {
//     const raw = localStorage.getItem(keyFor(userId, dateIso));
...
//     data: TaskRow[]
//   ) => {
//     localStorage.setItem(keyFor(userId, dateIso), JSON.stringify(data));
//   };
...
//     setTasks((t) => {
//       const updated = t.filter((r) => r.id !== id);
//       localStorage.setItem(
//         keyFor(userId, yyyyMMdd(selectedDate)),
...
//     await saveTasksToBackend(userId, yyyyMMdd(selectedDate), tasks);
...
//     setMonthShown(firstOfMonth);
//     setWeekStart(startOfWeek(firstOfMonth));
//     setSelectedDate(firstOfMonth);
//     setSelectedMonth(firstOfMonth.getMonth());
//     setSelectedYear(firstOfMonth.getFullYear());
//   };
...
// // import type {TaskRow} from "../api/dailyUpdateApi";
...
//   // add these two to match TaskEditorCard
//   __editing?: string;
//   __editingId?: string;
// };
...
//   // -------------------- FETCH FROM BACKEND --------------------
//   const fetchTasksFromBackend = async (userId: string, dateIso: string) => {
//     const record = await fetchInternDaily(userId, dateIso);
//     return record?.tasks || [];
//   };
...
//     const dateIso = yyyyMMdd(selectedDate);
//     const totalTime = `${hrs}h ${mins.toString().padStart(2, "0")}m`;
...
//     const result = await saveInternDaily(userId, dateIso, payload);
...
const DailyUpdate: React.FC = () => {
  ...
  // SAVE TO BACKEND
  // -----------------------------
  const saveTasksToBackend = async (userId: string, dateIso: string) => {
    const totalMinutes = tasks.reduce((acc, t) => {
      if (t.status === "completed") {
        const num = parseFloat(t.actualTime || "0");
        if (!isNaN(num) && num >= 0) {
    ...
    return await saveInternDaily(userId, dateIso, payload);
  };
